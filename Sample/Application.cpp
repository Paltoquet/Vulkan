...
void Application::setResizeWindow(bool resized)
{
...
void Application::createDescriptorLayout()
{
...
void Application::createImage(uint32_t width, uint32_t height, uint32_t mip_levels, VkSampleCountFlagBits nbSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
VkImageView Application::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mip_levels)
{
...
void Application::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mip_levels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
...
void Application::updateUniformBuffer(uint32_t currentImage)
{
    ...
    m_matrixBuffer.buffer.proj = glm::perspective(glm::radians(45.0f), m_swapChainExtent.width / (float)m_swapChainExtent.height, 0.1f, 10.0f);
    ...
    void* data;
    vkMapMemory(m_device, m_uniformBuffersMemory[currentImage], 0, sizeof(MatrixBuffer::BufferData), 0, &data);
...
{
    // Window mimization, go to background
    ...
    // wait all ressources no in used
    vkDeviceWaitIdle(m_device);
